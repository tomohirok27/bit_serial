...
def csv_write(filename, model_name, acc_max, train_batch, test_batch, num_worker, num_epochs, 
                lr, step_size, gamma, fixed_num):
    with open(filename, 'a') as f:
        writer = csv.writer(f)
        # writer.writerow([*args])
        writer.writerow(['model_name: {}, acc_max: {}'.format(model_name, acc_max)])
...
def plt_cum(y1, y2, y3, ylabel, name, key, ratio, e_th, t_th):
    fig = plt.figure()
    ...
    ax2.set_title('resnet18,4bit,cifar100,correct_rate_only')
    basename = os.path.splitext(os.path.basename(name))[0]
    date = datetime.datetime.now().strftime("%y%m%d")
    time = datetime.datetime.now().strftime("%H%M")
    filepath = "plt/%s/%s" % (ylabel, date)
    os.makedirs(filepath, exist_ok=True)
    plt.savefig("plt/%s/%s/%s_%s_%s_train%s_%s.png" % (ylabel, date, date, time, basename, int(100*t_th), ylabel))
    plt.close()


def plt_number_entropy(x, ylabel, name):
    fig = plt.figure()
    ...
    ax.plot((np.where(incorrect != 0)[0]),incorrect[incorrect != 0], marker='o', markersize=5, label='incorrect')
    # ax.plot(correct[~(correct == 0)], label='correct')
    # ax.plot(incorrect[~(incorrect == 0)], label='incorret')
    fig.legend()
    # fig.ylabel('%s' % ylabel)
    basename = os.path.splitext(os.path.basename(name))[0]
    date = datetime.datetime.now().strftime("%y%m%d")
    ...
    filepath = "plt/%s/%s" % (ylabel, date)
    os.makedirs(filepath, exist_ok=True)
    plt.savefig("plt/%s/%s/%s_%s_%s_train_%s.png" % (ylabel, date, date, time, basename, ylabel))
    plt.close()
    



def numpy_save(x, name, flag):
    basename = os.path.splitext(os.path.basename(name))[0]
    date = datetime.datetime.now().strftime("%y%m%d")
    time = datetime.datetime.now().strftime("%H%M")
    filepath = "numpy/%s/%s" % (flag, date)
    os.makedirs(filepath, exist_ok=True)
    filename = '%s_%s_%s' % (date, time, basename)
    np.save('numpy/%s/%s/%s' % (flag, date, filename), x)
